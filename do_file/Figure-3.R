...
#### Plots: TV tax compliance rate * 100, by decade ####
cities = c("Milano","Roma","Torino","Genova","Venezia","Firenze","Bologna")

# 1950 #
plot_tvtax_50 <- ggplot(data = maps) +
  geom_sf(aes(fill = d100f_tv_50), color = NA, size = 0) +  # Set color = NA to remove borders
  scale_fill_distiller(palette = "Blues", direction = 1, name = "TV tax compliance", na.value = "white", limits = c(0, 100), breaks = seq(0, 100, 20)) +  # Use the "Blues" palette with normal direction
...
# 1960 #
plot_tvtax_60 <- ggplot(data = maps) +
  geom_sf(aes(fill = d100f_tv_60), color = NA, size = 0) +  # Set color = NA to remove borders
...
# 1970 #
plot_tvtax_70 <- ggplot(data = maps) +
  geom_sf(aes(fill = d100f_tv_70), color = NA, size = 0) +  # Set color = NA to remove borders
  scale_fill_distiller(palette = "Blues", direction = 1, na.value = "white") +  # Use the "Blues" palette with normal direction
  geom_sf(data = regs_cn, fill = NA, color = "black", size = 1) +  # Add borders from regs_cn
  geom_text(data = maps[maps$COMUNE %in% cities,],
            aes(label = COMUNE, geometry = geometry), 
...
# 1980 #
plot_tvtax_80 <- ggplot(data = maps) +
  geom_sf(aes(fill = d100f_tv_80), color = NA, size = 0) +  # Set color = NA to remove borders
  scale_fill_distiller(palette = "Blues", direction = 1, na.value = "white") +  # Use the "Blues" palette with normal direction
  geom_sf(data = regs_cn, fill = NA, color = "black", size = 1) +  # Add borders from regs_cn
  geom_text(data = maps[maps$COMUNE %in% cities,],
            aes(label = COMUNE, geometry = geometry), 
...
# 1990 #
plot_tvtax_90 <- ggplot(data = maps) +
  geom_sf(aes(fill = d100f_tv_90), color = NA, size = 0) +  # Set color = NA to remove borders
  scale_fill_distiller(palette = "Blues", direction = 1, na.value = "white") +  # Use the "Blues" palette with normal direction
  geom_sf(data = regs_cn, fill = NA, color = "black", size = 1) +  # Add borders from regs_cn
  geom_text(data = maps[maps$COMUNE %in% cities,],
            aes(label = COMUNE, geometry = geometry), 
...
# 2000 #
plot_tvtax_00 <- ggplot(data = maps) +
  geom_sf(aes(fill = d100f_tv_00), color = NA, size = 0) +  # Set color = NA to remove borders
  scale_fill_distiller(palette = "Blues", direction = 1, na.value = "white") +  # Use the "Blues" palette with normal direction
  geom_sf(data = regs_cn, fill = NA, color = "black", size = 1) +  # Add borders from regs_cn
  geom_text(data = maps[maps$COMUNE %in% cities,],
            aes(label = COMUNE, geometry = geometry), 
